 ...
 * generated color.
 */
export function topNWithOther(items: { name: string; amount: number }[], maxSlots = 8): ColoredAmount[] {
  const sorted = [...items].filter((i) => i.amount > 0).sort((a, b) => b.amount - a.amount);
  if (sorted.length <= maxSlots) {
    return sorted.map((item, i) => ({ ...item, color: CATEGORICAL_PALETTE[i], isOther: false }));
  }
  const top = sorted.slice(0, maxSlots - 1);
  const rest = sorted.slice(maxSlots - 1);
  const otherAmount = rest.reduce((sum, r) => sum + r.amount, 0);
  return [
    ...top.map((item, i) => ({ ...item, color: CATEGORICAL_PALETTE[i], isOther: false })),
